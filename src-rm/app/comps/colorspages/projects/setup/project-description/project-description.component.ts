...
export class ProjectDescriptionComponent implements OnInit {
  ...
  constructor(public location: Location, private db: AngularFirestore) {
    this.items = db.collection('sectiontest1', ref => ref.orderBy('name')).valueChanges();
    this.items = db.collection('sectiontest2', ref => ref.orderBy('name')).valueChanges();
    this.items = db.collection('sectiontest3', ref => ref.orderBy('name')).valueChanges();
    this.items = db.collection('sectiontest4', ref => ref.orderBy('name')).valueChanges();
    this.items = db.collection('sectiontest5', ref => ref.orderBy('name')).valueChanges();
    this.items = db.collection('sectiontest6', ref => ref.orderBy('name')).valueChanges();
  }
  saveItem1() {
    console.log('saving');
